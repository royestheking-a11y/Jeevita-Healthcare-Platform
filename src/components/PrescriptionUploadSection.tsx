...
export function PrescriptionUploadSection({ onNavigate }: PrescriptionUploadSectionProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Simulate network delay for AI analysis processing
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Simulate OCR finding medicines from the image (e.g. Napa)
      const matched = medicines.filter(m => {
        const name = m.name.toLowerCase();
        return name.includes('napa') || name.includes('sergel');
      }).slice(0, 3);

      const verified = matched.map(m => ({
        name: m.name,
        dosage: m.strength || null,
        quantity: 1,
        form: m.category || null,
        verified: true,
        matchId: m.id,
        matchName: m.name,
        matchPrice: m.price,
      ...
      const unverified: AnalyzedMedicine[] = [];
      ...
      setAnalysisResult({ verified, unverified });
      
      if (verified.length > 0) {
        toast.success(`Found ${verified.length} verified medicines!`);
      } else {
        toast.info('Analysis complete. Please review the results.');
      }
  ...
  return (
    <>
      <section className="py-16 bg-linear-to-br from-amber-50 via-orange-50 to-amber-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            {/* Left: Content */}
            <div>
              <div className="inline-flex items-center gap-2 bg-linear-to-r from-amber-100 to-orange-100 border border-amber-300 text-amber-700 px-4 py-2 rounded-full mb-6">
              ...
              <div className="space-y-6 mb-8">
                {features.map((feature, index) => (
                  <div key={index} className="flex items-center gap-3">
                    <div className="shrink-0 w-10 h-10 bg-linear-to-br from-amber-500 to-orange-500 text-white rounded-lg flex items-center justify-center shadow-lg shadow-amber-500/30">
              ...
              <Button
                ...
                size="lg"
                className="bg-linear-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-8 shadow-lg shadow-amber-500/30"
              >
            ...
            <div className="relative">
              <div className="bg-white rounded-3xl p-8 shadow-2xl shadow-amber-500/20 border-2 border-amber-200">
                ...
                <div
                  onClick={handleOpenDialog}
                  className="border-2 border-dashed border-amber-300 rounded-2xl p-12 text-center bg-linear-to-br from-amber-50 to-orange-50 hover:border-amber-400 transition-colors group cursor-pointer"
                >
                  <div className="bg-linear-to-br from-amber-500 to-orange-500 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform shadow-lg shadow-amber-500/30">
                ...
                <div className="mt-6 space-y-3">
                  {[
                    ...
                    <div key={index} className="flex items-center gap-3 text-sm">
                      <div className="shrink-0 w-6 h-6 bg-linear-to-br from-amber-500 to-orange-500 rounded-full flex items-center justify-center shadow-md shadow-amber-500/30">
      ...
      <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
        <DialogContent className="max-w-4xl sm:max-w-4xl max-h-[90vh] overflow-y-auto bg-white">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-2xl font-bold bg-linear-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent py-1">
              ...
              <div className="flex flex-col gap-4">
                <Button
                  ...
                  className={`w-full h-14 text-lg font-bold text-white shadow-xl transition-all duration-300 rounded-xl flex items-center justify-center gap-3 ${prescriptionImage && !analysisResult && !analyzing
                    ? 'bg-linear-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-600 hover:to-orange-600 shadow-amber-500/30 animate-pulse scale-[1.02]'
                    : 'bg-gray-300 cursor-not-allowed'
                    } ${analyzing ? 'bg-linear-to-r from-amber-400 to-orange-400 cursor-wait' : ''}`}
                >
                  {analyzing ? (
                    <>
                        ...
                        {analysisResult.verified.map((item, idx) => (
                          <div key={idx} className="group bg-white p-4 rounded-xl border border-green-100 shadow-sm hover:shadow-md hover:border-green-300 transition-all flex flex-col">
                            <div className="flex items-start gap-4">
                              <div className="shrink-0 w-16 h-16 bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
                                {item.matchImage ? (
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-bold text-gray-900 text-lg leading-tight">{item.matchName}</p>
                            ...
                            <Button
                              ...
                              className="w-full mt-4 bg-linear-to-r! from-amber-500! to-orange-500! hover:from-amber-600! hover:to-orange-600! text-white shadow-amber-200 shadow-md rounded-lg h-11 flex items-center justify-center gap-2 text-sm font-semibold transition-all duration-300 transform hover:scale-[1.02] border-none"
                            >
